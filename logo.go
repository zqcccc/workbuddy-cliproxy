// logo.go holds the provider icon the plugin publishes to the management UI.
//
// The host has no built-in WorkBuddy icon: its management panel resolves a
// provider's icon from a hardcoded table for the providers it ships with
// (claude, codex, gemini, kimi, qwen, ...) and falls back to a generic plug
// glyph for anything else. The one channel a plugin controls is the `logo`
// field of its registration metadata, which the panel reads for the plugin's
// OAuth card, the plugin list, and the sidebar.
//
// The icon is inlined as a base64 data URL rather than referenced over HTTP
// for two reasons. First, the panel is rendered in the operator's browser,
// which may not be able to reach the same hosts the server can; a data URL
// always resolves. Second, the host passes this value through
// html.EscapeString, which mangles the `<` of a raw inline SVG but leaves
// base64 (A-Za-z0-9+/=) untouched.
//
// Source: the official WorkBuddy mark, https://download.codebuddy.ai/
// web/workbuddy/.../assets/logo.svg — a 40x40 rounded-square teal badge with
// the white "W" glyph. Only Figma's redundant display-p3 `style` attributes
// were stripped; the geometry is byte-for-byte the published file.

package main

import "encoding/base64"

// logoSVG is the icon source kept in the tree so it can be re-derived and
// diffed; the compiled-in value below is what the host actually sees.
...
<circle cx="31.3411" cy="43.6183" r="12.8704" fill="#FFE355" fill-opacity="0.49"/>
</g>
...
// logoDataURL is logoSVG base64-encoded for the registration metadata. The
// encoding runs once at init so the SVG above stays the single source of truth.
var logoDataURL = "data:image/svg+xml;base64," + base64.StdEncoding.EncodeToString([]byte(logoSVG))
