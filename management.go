...
func renderQuotaHTML(snapshot quotaSnapshot, masked bool) string {
	...
	for _, acc := range snapshot.Accounts {
		...
		if len(acc.Packages) > 0 {
			b.WriteString(`<table><thead><tr><th>资源包</th><th>剩余 / 总量</th><th>周期截止</th></tr></thead><tbody>`)
			for _, pkg := range acc.Packages {
				b.WriteString(`<tr><td>` + html.EscapeString(pkg.Name) + `</td><td class="num">` +
					formatCredits(pkg.Left) + ` / ` + formatCredits(pkg.Total) + `</td><td>` +
					html.EscapeString(pkg.CycleEnd) + `</td></tr>`)
			}
